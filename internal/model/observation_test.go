...
func TestValidateBackendObservationVerifiesBoundsAndArtifacts(t *testing.T) {
	...
	valid := training.Observation{
		...
		Evaluations:        []training.Evaluation{{Step: 2, Tokens: 128, Metrics: map[string]float64{"heldout_loss": loss}}},
	...
	for _, test := range []struct {
		...
		{name: "evaluation", mutate: func(value *training.Observation) { value.Evaluations[0].Metrics["heldout_loss"] = math.Inf(1) }, want: "metric"},
		...
		t.Run(test.name, func(t *testing.T) {
			...
			candidate.Evaluations[0].Metrics = map[string]float64{"heldout_loss": loss}
