...
func newModelCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "train-tokenizer [index-path...]", "Train a deterministic tokenizer artifact", "Builds a byte-fallback tokenizer from a bounded deterministic sample of a distributable index selection. The artifact pins the corpus BOM and reports compression against r50k_base.", cobra.ArbitraryArgs, runModelTrainTokenizer,
			...
			booleanFlag("force", "replace an existing tokenizer artifact")),
		leaf(state, "evaluation-bom <definition.yaml> <output.json>", "Pin an immutable evaluation corpus", "Resolves a versioned evaluation definition, requires distributable corpus evidence, and writes an immutable evaluation BOM.", cobra.ExactArgs(2), runModelEvaluationBOM,
			booleanFlag("force", "replace an existing evaluation BOM")),
		leaf(state, "gate <name> <evaluation-bom.json> <results.json> <output.json>", "Apply contamination and metric promotion gates", "Checks every completed real training corpus against the pinned evaluation corpus, verifies externally produced metric results are bound to that BOM, and writes a deterministic gate report.", cobra.ExactArgs(4), runModelGate,
			booleanFlag("force", "replace an existing gate report")),
		leaf(state, "train <name> [index-path...] | <name> <compose-file>", "Train a model from indexed corpora or a compose", "A compose creates an absent model or trains an existing model with the same architecture. With no input, WALDO trains an existing model on the entire resolved index. --hostfile launches a multi-host TorchTitan job: the first listed host is the local rank-0 host, and remaining hosts are launched over non-interactive SSH. In a Fuzzball generic multi-node job, MULTINODE_HOSTLIST_NOSLOTS and MULTINODE_SSH_WRAPPER provide the same topology and launcher automatically. WALDO stages and verifies its exact binary and a homogeneous TorchTitan/GPU topology. Each node mirrors the immutable stage shards once into its local cache and prepares only the sequences assigned to its local ranks. Manual --nodes and rendezvous flags remain a compatibility interface.", modelTrainArgs, runModelTrain,
			...
			textFlag("hostfile", "", "launch a multi-host TorchTitan run on the listed hosts"),
			integerFlag("rendezvous-port", 29500, "primary TorchTitan rendezvous port used with --hostfile"),
		...
		leaf(state, "train-worker", "Join a multi-node training run as a secondary node", "Internal worker used by the hostfile launcher. Manual --nodes, --node-rank, and rendezvous flags remain available for compatibility. Each node materializes the immutable stage corpus into its own cache and prepares only the sequences assigned to its local ranks.", cobra.NoArgs, runModelTrainWorker,
			...
			booleanFlag("plan-stdin", "read launcher-delivered stage plans from standard input"),
			booleanFlag("check", "verify and report this host's TorchTitan capabilities"),
			textFlag("scratch", "", "launcher-managed local training scratch path"),
			textFlag("cache-root", "", "launcher-provided node-local verified object cache"),
			textFlag("cache-scratch", "", "launcher-provided node-local download and prepared-data scratch"),
			integer64Flag("cache-max-bytes", 0, "launcher-provided node-local cache byte limit"),
			repeatedTextFlag("cache-mirror", "launcher-provided lookaside mirror"),
			textFlag("nccl-interface", "", "launcher-provided NCCL network interface"),
			textFlag("nccl-hca", "", "launcher-provided NCCL RDMA device")),
